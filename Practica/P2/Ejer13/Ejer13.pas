...
procedure actualizarM(var mae: maestro; var det1, det2: detalle);
var
	...
	while(min.destino <> valorAlto) do begin
		...
		while(ver.destino = min.destino) do begin
			...
			while(ver.destino = min.destino) and (ver.fecha = min.fecha) do begin
				...
				read(mae, regV);
				while(regV.destino <> ver.destino) and (regV.fecha <> ver.fecha) and (ver.hora <> regV.hora) do
					read(mae, regV);
...
procedure leerM(var mae:maestro; var regV: vuelos);
begin
	if(not eof(mae)) then
		read(mae, regV)
	else
		regV.destino:= valorAlto;
end;

procedure generarLconM(var mae: maestro; var L: lista);
var
	cantU: integer;
	regV: vuelos;
begin
	...
	leerM(mae, regV);
	while(regV.destino <> valorAlto) do begin
